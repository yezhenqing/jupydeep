...
export default function CountdownTimer({
  ...
  if (timeLeft <= 0) {
    return (
      <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-medium text-xs ${className}`}>
  ...
  return (
    <div className={`inline-flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-slate-950/60 border border-slate-800/80 shadow-xl backdrop-blur-md ${className}`}>
      ...
      <div className="font-mono text-base font-bold tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-indigo-200 via-white to-purple-200">
        <span>{hours}</span>
        <span className="mx-0.5 text-indigo-400/60 font-sans animate-pulse">:</span>
        <span>{minutes}</span>
        <span className="mx-0.5 text-indigo-400/60 font-sans animate-pulse">:</span>
        <span>{seconds}</span>
      </div>
